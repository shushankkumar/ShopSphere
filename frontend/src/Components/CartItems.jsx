...
const CartItems = () => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-4 py-10 sm:px-6 lg:px-8">
      <div className="hidden grid-cols-[0.9fr_1.6fr_1fr_1fr_1fr_0.6fr] items-center gap-4 rounded-2xl bg-slate-100 px-6 py-4 text-sm font-semibold uppercase tracking-wide text-slate-600 md:grid">
        <p>Product</p>
        <p>Title</p>
        <p>Price</p>
        <p>Size</p>
      ...
      {all_product.map((e) => {
        if (cartItems[e.id] > 0) {
          return (
            <div key={e.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:px-6 md:py-5">
              <div className="grid gap-4 md:grid-cols-[0.9fr_1.6fr_1fr_1fr_1fr_0.6fr] md:items-center">
                <div className="flex items-center gap-4">
                  <img src={e.image} alt={e.name} className="h-24 w-20 rounded-2xl bg-slate-100 p-2 object-cover" />
                  <div className="md:hidden">
                    <p className="text-base font-semibold text-slate-900">{e.name}</p>
                    <p className="mt-1 text-sm text-slate-500">Rs {e.new_price}</p>
                  </div>
                </div>
                <p className="hidden text-base font-semibold text-slate-900 md:block">{e.name}</p>
                <p className="text-sm font-medium text-slate-600 md:text-base">Rs {e.new_price}</p>
                ...
                <p className="text-sm font-semibold text-slate-900 md:text-base">Rs {e.new_price * cartItems[e.id]}</p>
                <button
                  type="button"
                  onClick={() => {
                    removeFromCart(e.id)
